...
class PersistentBackendStage12GapsScanSmokeTests(unittest.TestCase):
    def test_gaps_scan_is_internal_and_not_exposed_as_public_cli_command(self) -> None:
        ...
        self.assertNotIn("gaps:scan", main)
        self.assertNotIn("--create-backfill", main)
        self.assertNotIn("Gaps scan report", main)
        self.assertNotIn("run_gaps_scan", maintenance)
        self.assertNotIn("gaps:scan", readme)
