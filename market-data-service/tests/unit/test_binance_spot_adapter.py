...
class BinanceSpotAdapterTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_fetch_closed_candles_supports_all_canonical_timeframes(self) -> None:
        ...
        for timeframe, secs in durations.items():
            with self.subTest(timeframe=timeframe):
                open_ms = 1784016000000
                close_ms = open_ms + secs * 1000 - 1
                transport = FakeTransport([[_kline(open_ms, close_ms)]])
                adapter = BinanceSpotAdapter(
                    _config(),
                    transport=transport,
                ...
                self.assertEqual(transport.requests[0][1]["interval"], timeframe)
    ...
    async def test_fetch_closed_candles_paginates_multiple_pages(self) -> None:
        transport = FakeTransport(
            [
                [
                    _kline(1784008800000, 1784012399999),
                    _kline(1784012400000, 1784015999999),
                ],
                [
                    _kline(1784016000000, 1784019599999),
                    _kline(1784019600000, 1784023199999),
                ],
                [
                    _kline(1784023200000, 1784026799999),
                ],
            ]
        )
        ...
        self.assertEqual([candle.open_time.hour for candle in candles], [6, 7, 8, 9, 10])
        self.assertEqual(len(transport.requests), 3)
        self.assertEqual(transport.requests[0][1]["startTime"], 1784008800000)
        self.assertEqual(transport.requests[1][1]["startTime"], 1784016000000)
        self.assertEqual(transport.requests[2][1]["startTime"], 1784023200000)
        for _, params in transport.requests:
            self.assertEqual(params["endTime"], 1784026800000)
