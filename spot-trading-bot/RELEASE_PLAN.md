...
При Bybit taker fee 0.1% і буфері slippage 0.1%:
...
| 4 | Hardening виконання | блокує реліз | Фаза 3 |
...
### 1.3. Timezone у scheduler використовує локальний час
...
Якщо сервер у UTC, а бот налаштований на 00:00:01 Kyiv time (UTC+3), він запуститься
на 3 години раніше ніж очікується.
Рішення: або завжди UTC, або явно читати `APP_TIMEZONE` з config і конвертувати.
...
Стратегія CVD не використовує, але це може стати проблемою при доданні нових фіч.

**Рішення:** при upsert перераховувати CVD на основі попередніх значень у БД,
або позначити CVD як legacy-колонку і не оновлювати її при upsert.
...
`lru_cache` на instance-методі не прив'язаний до `self` — кеш є глобальним на весь процес.
Якщо Bybit змінить лімітний розмір ордера або tick_size, бот дізнається про це лише після рестарту.
Для spot-торгівлі фільтри рідко змінюються, але кеш без TTL ризикований для довготривалих процесів.

**Рішення:** замінити на `functools.cached_property` або кеш з TTL (1 година).
...
навіть якщо прибуток менший за комісію Bybit (0.1% taker + 0.1% taker = 0.2% round-trip).
Тобто при `MIN_PROFIT_RATIO = 0` і комісіях бот реалізує збиток на кожній угоді.
...
**Нова міграція:** `migrations/003_create_4h_candles.sql`
...
**Зміни в `utils/create_tables.py`:**
Додати `CREATE_4H_CANDLES_TABLE_SQL` і цикл по символах для H4-таблиць.
...
### 3.6. Scheduler: перехід від daily до 4H
...
Поточний `wait_until_next_daily_run` замінюється на `wait_until_next_interval_run`:
...
Candle close times for 4H (UTC): 03:59, 07:59, 11:59, 15:59, 19:59, 23:59.
Бот запускається через 1 секунду після цільового часу запуску (04:00:01, 08:00:01, ... 00:00:01).
...
analyze --timeframe 4h    — one-off 4H analysis cycle
```
...
**Рішення:** перевіряти `order_ledger` на наявність виконаного ордера за поточний candle_id
(= `signal.close_time`) перед виконанням:
...
# domain/execution.py — без змін в логіці, лише правильний дефолт:
...
Цей шар відсіює сигнали ще до відправки на біржу.
...
ціна впала нижче беззбиткового рівня (slippage між candle close і market execution).
...
Після кожного SELL ордера перевірити фактичну `executed_price` відносно `avg_entry_price`.
Якщо через slippage угода виявилась збитковою — записати в ledger окрему подію і надіслати alert:
...
async def _apply_position_update(self, decision, position_state, executed_price, ...):
    if decision.action == "sell":
        ...
        if realized_pnl < Decimal("0"):
            logger.error(
                "no_loss_violation symbol=%s executed_price=%s avg_entry=%s pnl=%s",
                decision.symbol, executed_price, position_state.avg_entry_price, realized_pnl,
            )
            # Відправити Telegram alert (критичний рівень)
```

Цей шар не запобігає збитку (ордер вже виконаний), але фіксує порушення для аудиту.
...
| `no_loss_violation` | шар 3 виявив збиток після виконання |
...
```env
# .env / .env.example
MIN_PROFIT_RATIO=0.01         # мінімальний відсоток прибутку над avg_entry (≥ 0.003)
...
# Захист від помилкової конфігурації нижче комісій:
_MIN_ALLOWED_PROFIT_RATIO = Decimal("0.003")
if MIN_PROFIT_RATIO < _MIN_ALLOWED_PROFIT_RATIO:
    raise ValueError(
        f"MIN_PROFIT_RATIO={MIN_PROFIT_RATIO} нижчий за мінімально допустимий "
        f"{_MIN_ALLOWED_PROFIT_RATIO} (менше round-trip комісій Bybit)"
    )
```
...
# Шар 1: продаж дозволено вище порогу
def test_sell_allowed_when_price_above_min_profit_ratio():
...
def test_no_loss_guard_blocks_sell_when_market_price_dropped():
    # signal_price=101 (проходить шар 1), але поточна ціна=99 (нижче порогу)
    ...
    assert result.action == "skip"
    assert result.reason == "no_loss_guard_infrastructure"

# Конфігурація: MIN_PROFIT_RATIO нижче комісій не дозволяється
...
- [ ] 4.5 Шар 3: post-execution audit — логування та alert при фактичному збитку
- [ ] 4.5 Нові статуси `blocked_no_loss` / `no_loss_violation` у `order_ledger`
- [ ] 4.5 Міграція 004: колонки `realized_pnl_usdt`, `realized_pnl_pct`, `no_loss_check_price`
...
- [ ] 3.2 Міграція 003 для H4-таблиць
- [ ] 3.3 `sync-4h` команда
...
Тиждень 2: Фаза 4.5 (no-loss 3 шари + міграція 004 + тести 6.4)
...
Тиждень 6: Фаза 4.1–4.5 (hardening + no-loss) + Фаза 6.1
