...
### 2.6. Production env precedence має бути явно зафіксований

Production-запуск має брати змінні з `.env.production`. Якщо цього файлу немає,
дозволений fallback на `.env`, який використовується для локальних і тестових значень.

Поточний стан реалізації:
- `utils/env.py` вже завантажує `.env.production` перед `.env`
- `utils/config.py` викликає `load_project_env()` при імпорті конфігурації

**Рішення:** зберегти цей контракт як обов'язкову production-поведінку:
- `.env.production` має пріоритет над `.env`
- `.env` не вважається production-файлом і може містити тестові/локальні значення
- `.env.example` має описувати обидва файли та очікуваний порядок fallback
...
- [x] 1.1 Виправлено `exchange_order_id` extraction
- [x] 1.2 Синхронізація передає `days=3` замість 1095
- [x] 1.3 Scheduler використовує UTC
- [x] 1.4 `run_many` ізолює помилки символів
- [x] 1.5 Position upsert + ledger в одній транзакції
- [x] 2.1 CVD позначено як legacy-колонку і прибрано з runtime signal input
- [x] 2.2 `get_symbol_filters` використовує TTL cache замість безстрокового `lru_cache`
- [x] 2.3 Live startup reconciliation увімкнено перед scheduler wait
- [x] 2.4 `MIN_PROFIT_RATIO` = 0.01 (1%) за замовчуванням
- [x] 2.5 Файл `.env.example` присутній
- [x] 2.6 Production env precedence зафіксований: `.env.production` → `.env`
- [x] 4.1 Retry для Bybit API calls
- [x] 4.2 Перевірка статусу ордера після виставлення
- [x] 4.3 Graceful shutdown
- [x] 4.4 Захист від дублювання ордерів за `symbol + action + signal_timeframe + signal_candle_id`
- [x] 6.1 Всі існуючі тести проходять

### No-Loss (must-have перед першим prod-запуском)

- [x] 4.5 Шар 1: `MIN_PROFIT_RATIO=0.01` як захищений дефолт з валідацією при старті
- [x] 4.5 Шар 2: pre-execution guard у `BybitSpotExecutor._execute_trade` з fetch поточної ціни
- [x] 4.5 Шар 3: post-execution audit — логування та audit-status при SELL нижче мінімального профіту 1%
- [x] 4.5 Нові статуси `blocked_no_loss` / `no_loss_violation` у `order_ledger`
- [x] 4.5 Міграція 004: колонки `realized_pnl_usdt`, `realized_pnl_pct`, `no_loss_check_price`, `signal_timeframe`, `signal_candle_id`
- [x] 4.5 `NO_LOSS_GUARD_ENABLED` env var задокументовано у `.env.example`
- [x] 6.4 Unit-тести всіх трьох шарів no-loss (домен, інфраструктура, конфігурація)

### Multi-timeframe (нова функція)

- [x] 3.2 Базова міграція 003 для H4-таблиць
- [x] 3.2 Runtime wiring для створення/читання H4-таблиць у live flow
- [x] 3.3 `sync-4h` команда
- [x] 3.4 `MultiTimeframeMarketDataProvider`
- [x] 3.5 `MultiTimeframeSpotPlanner` з D1 regime filter
- [x] 3.6 H4 scheduler (6 разів на день)
- [x] 3.7 Синхронізатор параметризований по timeframe
- [x] 3.8 Нові env-змінні задокументовані
- [x] 3.9 CLI-команди `sync-4h`, `sync-full`, `analyze --timeframe 4h`
- [x] 6.2 Unit-тести multi-timeframe плану
- [ ] 6.3 Тести для фіксованих багів

### Операційна готовність

- [x] 5.1 `print` замінено на `logging`
- [x] 5.2 Telegram-нотифікації для угод (включаючи `blocked_no_loss` alerts)
- [x] 5.3 PnL-колонки в `order_ledger`
- [x] 5.4 Періодичний PnL summary після scheduled cycle
- [x] 5.5 Health-check endpoint `/health`
- [ ] 6.5 Integration-тести з реальною БД
- [ ] 6.6 E2E dry-run тест
