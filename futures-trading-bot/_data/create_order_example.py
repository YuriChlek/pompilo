...
def generate_signature(params, secret):
    """
    /**
     * Будує HMAC-SHA256 підпис для параметрів запиту Bybit.
     * @param {dict} params - Відсортований набір параметрів, що формують рядок запиту.
     * @param {str} secret - Секретний API-ключ, який використовується для підпису.
     * @returns {str} - Гекс-рядок підпису.
     */
    """
...
def check_balance():
    """
    /**
     * Перевіряє баланс демо-акаунту Bybit через REST API.
     * @returns {float|None} - Значення ``totalEquity`` або ``None`` у разі помилки API.
     * @edgecases Поразки виклику повторюються до трьох разів із експоненційною затримкою.
     */
    """
    params = {
        "api_key": API_KEY,
...
def fetch_current_price(symbol="BTCUSDT"):
    """
    /**
     * Отримує останню ринкову ціну для заданого символу з демо-оточення Bybit.
     * @param {str} symbol - Торгова пара (наприклад, ``BTCUSDT``).
     * @returns {float|None} - Поточна ціна ``lastPrice`` або ``None`` при збої.
     */
    """
    params = {
        "category": "linear",
...
def create_order(symbol, side, qty, sl_target, tp_target):
    """
    /**
     * Відправляє маркет-ордер на демо Bybit із необов'язковими стоп-лосом та тейк-профітом.
     * @param {str} symbol - Торгова пара (наприклад, ``SOLUSDT``).
     * @param {str} side - Напрямок угоди: ``Buy`` або ``Sell``.
     * @param {float|str} qty - Розмір позиції.
     * @param {float|str|None} sl_target - Рівень стоп-лосу, може бути ``None``.
     * @param {float|str|None} tp_target - Рівень тейк-профіту, може бути ``None``.
     * @returns {str|None} - ID створеного ордера або ``None`` у разі помилки.
     * @edgecases API-помилки супроводжуються повторними спробами; при вичерпанні ретраїв повертається ``None``.
     */
    """
    params = {
        "category": "linear",
