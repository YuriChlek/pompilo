...
export class JwtAdminAuthStrategy extends PassportStrategy(Strategy, 'admin-jwt') {
    ...
    validate(accessTokenPayload: AccessTokenPayload): AccessTokenPayload | null {
        if (
            accessTokenPayload.realm !== 'admin' ||
            (accessTokenPayload.role !== UserRoles.PLATFORM_ADMIN &&
                accessTokenPayload.role !== UserRoles.SUPER_ADMIN)
        ) {
            throw new UnauthorizedException('Invalid realm or role for admin strategy.');
        }
